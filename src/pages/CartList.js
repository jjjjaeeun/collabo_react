...
function App({ user }) {
    ...
    // 전체 선택 체크박스를 Toggle 함
    const toggleAllCheckBox = (isAllCheck) => {
        // isAllCheck: 전체 선택 체크박스의 boolean 값
        setCartProducts((previous) => {
            // 모든 객체(카트상품)들의 나머지 속성은 보존하고, 체크상태(checked)를 천제 선택 체크상태와 동일하게 설정함 (전체 선택 선택스 true, 해제시 false)
            const updatedProducts = previous.map((product) => ({
                ...product,
                checked: isAllCheck
            }));

            // 비동기적 렌더링 문제로 수정된 updatedProducts 항목을 매개로 변수로 넘겨야 정상적으로 작동
    ...
    const changeQuantity = async (cartProductId, quantity) => {
        // NaN: Not a Number
        if (isNaN(quantity)) { // 숫자 형식이 아니면(참이면 숫자가 아님)
            setCartProducts((previous) => {
        ...
        try {
            ...
            // http://localhost:9000/cart/edit/100?quantity=10 이런식으로 요청함
            const url = `${API_BASE_URL}/cart/edit/${cartProductId}?quantity=${quantity}`;

            // patch 동작은 전체가 아닌 일부 데이터를 변경하고자 할 때 사용
            // 스프링의 WebConfig 클래스안의 addCorsMappings() 메소드를 참조바람
            const response = await axios.patch(url);

            console.log(response.data || '');

            // cartProducts의 수량 정보를 갱신함
            setCartProducts((previous) => {
                const updatedProducts = previous.map((product) =>
                    product.cartProductId === cartProductId
        ...
        <Container className="mt-4">
            ...
            <Table striped bordered>
                <thead>
                    <tr>
                        <th style={thStyle}>
                            {/* 전체 선택 체크 박스의 체크 상태를 toggleAllCheckBox 함수에 전달 */}
                            <Form.Check
                                type="checkBox"
                                label="전체 선택"
                        ...
                        cartProducts.map((product) => (
                            <tr key={product.cartProductId} >
                                <td className="text-center align-middle">
                                    {/* 어떤 체크인지 식별하기 위하여 toggleCheckBox 함수에 cartProductId를 넘겨줌 */}
                                    <Form.Check
                                        type="checkbox"
