...
상품 목록을 상품 id 역순으로 읽어서 화면에 전체를 보여줌
하나의 행에 3개의 열씩 보여줌
...
function App({ user }) {
    ...
    const [paging, setPaging] = useState({
        ...
        // 자바의 SerchDto 클래스 연관 필드
        searchDateType: 'all', // 기간 검색 콤보박스
        category: '', // 검색하고자 하는 특정 카테고리 콤보박스
        searchMode: '', // 상품 검색모드 콤보박스
        searchKeyword: '', // 검색 키워드 입력상자
    });
    ...
    useEffect(() => {
        ...
        axios.get(url, parameters)
            .then((response) => {
                ...
                // 사용자가 pagination 항목을 클릭하였으므로, 페이징 정보를 업데이트 해야함.
                // 0 base 이므로 코드 작성게 유의
                // previous 이전 페이징 정보
                setPaging((previous) => {
                    ...
                    // 만약 pageSize의 값이 고정적이라면 할당 받지 않아도 됨
                    // 단, 가변적인 경우 반드시 할당 받아야함
    ...
    // 이 함수는 관리자 모드일때 보여주는 수정과 삭제를 위한 버튼을 생성해주는 함수
                ...
                <Button
                    ...
                    onClick={async (event) => {
                        ...
                        try { // 상품을 삭제 후 다시 상품 목록 페이지를 보여줍니다.
                            // 주의) 상품을 삭제하려면 반드시 primary key인 상품의 id를 넘겨주어야함
                            await axios.delete(`${API_BASE_URL}/product/delete/${item.id}`);

                            // alert 함수(modal 통신)와 비동기 통신 사용시, 화면 갱신에 유의 하기
                            alert(`'${item.name}' 상품이 삭제되었습니다.`);

                            // 삭제된 id를 배제하고, 상품 목록 state를 다시 갱신함
    ...
    return (
        <Container className="my-4">
            ...
            <Row>
                ...
                {products.map((item) => (
                    <Col key={item.id} md={4} className="md-4" >
                                ...
                                <table style={{ width: '100', borderCollapse: 'collapse', border: 'none' }}>
                                    <tbody>
                                        <tr>
                                            ...
                                            {/* rowSpan 속성은 행방향으로 병합시 사용 (반대 colSpan) 
                                            textAlign: 수평 정렬 방식, 
                                            verticalAlign: 수직 정렬 방식 */}
